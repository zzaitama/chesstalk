...
- **Direct Anthropic API calls** from the browser (`https://api.anthropic.com/v1/messages`)
...
## Key Decisions
- Client-side API calls: no backend, no proxy. The user provides their own Anthropic API key via the UI.
...
4. **Do not add a backend or proxy** — direct API calls are intentional.
